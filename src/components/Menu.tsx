      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8">
            <h3 className="text-3xl font-bold text-gray-900 mb-6 border-b-4 border-marko-orange pb-3">
              Choose Your Base
            </h3>
            <div className="space-y-4">
              {bases.map((base, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 hover:bg-marko-light rounded-lg transition-colors">
                  ...
                  <span className="text-2xl font-bold text-marko-orange">{base.price}</span>
                </div>
              ))}
            </div>
            <p className="text-sm text-gray-500 mt-4 italic">*Plate comes with mixed lettuce and chips or rice</p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <div className="space-y-3">
              {schnitzels.map((schnitzel, idx) => (
                <div key={idx} className="flex items-center p-3 hover:bg-marko-light rounded-lg transition-colors">
                  <div className="w-3 h-3 bg-gradient-marko rounded-full mr-3"></div>
                  <span className="text-lg text-gray-800">{schnitzel}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <div className="space-y-4">
              {grillItems.map((item, idx) => (
                <div key={idx} className="p-3 hover:bg-marko-light rounded-lg transition-colors">
                  <div className="flex items-start">
                    <div className="w-3 h-3 bg-gradient-marko rounded-full mr-3 mt-2"></div>
                    <div>
                      <span className="text-lg font-semibold text-gray-800 block">{item.name}</span>
                      <span className="text-sm text-gray-600 italic">{item.note}</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <div className="space-y-4">
              {extras.map((extra, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 hover:bg-marko-light rounded-lg transition-colors">
                  <span className="text-lg text-gray-800">{extra.name}</span>
                  <span className="text-xl font-bold text-marko-orange">{extra.price}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              {breakfast.map((item, idx) => (
                <div key={idx} className="p-3 hover:bg-marko-light rounded-lg transition-colors">
                  <div className="flex items-start justify-between mb-1">
                    <span className="text-lg font-semibold text-gray-800 flex-1">{item.name}</span>
                    <span className="text-xl font-bold text-marko-orange ml-4">{item.price}</span>
                  </div>
                  {item.note && <span className="text-sm text-gray-600 italic">{item.note}</span>}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8 lg:col-span-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {drinks.map((drink, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 hover:bg-marko-light rounded-lg transition-colors">
                  <span className="text-lg text-gray-800">{drink.name}</span>
                  <span className="text-xl font-bold text-marko-orange">{drink.price}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            ...
            >
              <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-marko text-white rounded-full mb-4">
                {feature.icon}
              </div>
